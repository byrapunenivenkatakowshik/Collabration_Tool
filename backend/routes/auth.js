  ...
  try {
    ...
    if (email.length > 20) {
      return res.status(400).json({ 
        ...
        details: 'Email must be no more than 20 characters'
      });
    }
    ...
    if (name.length > 15) {
      return res.status(400).json({ 
        error: 'Name too long',
        details: 'Username must be no more than 15 characters'
      });
    }
    ...
    if (password.length > 10) {
      return res.status(400).json({ 
        ...
        details: 'Password must be no more than 10 characters'
      });
    }
